...
export function HeroTerminalDemo({
  ...
  const demos = API_DEMOS.map((demo, index) => ({
    ...demo,
    endpoint: resolvedEndpoints[index] ?? demo.endpoint,
  }))

  useEffect(() => {
    setActiveIndex((prev) => (prev < demos.length ? prev : 0))
  }, [demos.length])
  ...
  const origin = baseUrl.replace(/\/$/, '') || `https://${displayHost}`
    ...
    <div className={cn('mx-auto w-full max-w-2xl', className)}>
      <div
        className={cn(
          ...
          'border-white/[0.45] bg-white/[0.88] shadow-[0_20px_50px_-25px_rgba(15,23,42,0.18)]',
          'dark:border-white/[0.12] dark:bg-[#0b0f17]/88 dark:shadow-[0_20px_60px_-25px_rgba(0,0,0,0.7)]'
        )}
      >
        <div
          className={cn(
        ...
        <div className='grid h-[400px] grid-rows-[235px_minmax(0,1fr)] font-mono text-[12.5px] leading-[1.55]'>
          <RequestBlock demo={demo} transitioning={transitioning} origin={origin} />
...
function RequestBlock(props: {
  ...
  const { demo, transitioning, origin } = props
    ...
    <div className='relative px-5 py-4'>
      ...
      >
        <CodeLine>
          ...
          <StringText>&quot;{origin}{demo.endpoint}&quot;</StringText>{' '}
          <Muted>{'\\'}</Muted>
        </CodeLine>
