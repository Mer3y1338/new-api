    ...
    <div className='space-y-2.5'>
      ...
      <div className='grid grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] gap-1.5'>
        <div className='min-w-0 rounded-md bg-muted/20 px-2 py-1.5'>
          ...
          <MobileLogTimeStatus
            createdAt={(cells.get('created_at')?.row.original as Record<string, unknown> | undefined)?.created_at}
            type={(cells.get('created_at')?.row.original as Record<string, unknown> | undefined)?.type}
          />
        </div>
        <SummaryField
          label={t('Channel')}
